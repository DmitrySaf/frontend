...
export const mockRequest = async <T>(
  ...
  console.log(`🚀 ${context} Mock Request starting - delay: ${delay}ms, success rate: ${successRate * 100}%`)
  ...
  console.log(`🟢 ${context} Mock Request Success after ${duration}ms:`, {
    ...
    hasData: !!data,
    ...(data && typeof data === 'object' && 'length' in data ? { length: (data as any).length } : {}),
    ...(data && typeof data === 'object' && 'projects' in data ? { projectsCount: (data as any).projects?.length } : {})
  })
