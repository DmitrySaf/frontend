...
/* Центрирование — флексом на обёртке, а НЕ через translate(-50%) на самой карточке:
   ...
   <md — bottom sheet (этап 17.7): карточка прижата к низу во всю ширину, углы скруглены
   только сверху, вход/выход — выезд снизу (@keyframes sheet-in/out) по iOS-кривой
   --ease-drawer. Свайп-вниз = «клик снаружи» (§4.1 DESIGN.md): жест живёт на ручке
   и доступен только при dismissOnOutside; закрывает через скрытый Close-прокси. */
const DialogContent = React.forwardRef<
  React.ElementRef<typeof DialogPrimitive.Content>,
  DialogContentProps
>(({ className, children, showClose = true, dismissOnOutside = true, ...props }, ref) => {
  const closeProxyRef = React.useRef<HTMLButtonElement>(null);
  ...
  const handlePointerDown = (event: React.PointerEvent<HTMLDivElement>) => {
    dragState.current = { startY: event.clientY, active: true };
    setIsSnapping(false);
    event.currentTarget.setPointerCapture(event.pointerId);
  };
  const handlePointerMove = (event: React.PointerEvent<HTMLDivElement>) => {
    if (!dragState.current.active) return;
    setOffset(Math.max(0, event.clientY - dragState.current.startY));
  };
  const handlePointerEnd = () => {
    if (!dragState.current.active) return;
    dragState.current.active = false;
    if (offsetRef.current > 90) {
      // Выходная анимация подхватит с текущего смещения — рывка не будет
      closeProxyRef.current?.click();
      return;
    }
  ...
  return (
    <DialogPortal>
      <DialogOverlay />
      <div className="pointer-events-none fixed inset-0 z-[var(--z-modal)] flex items-center justify-center p-4 max-md:items-end max-md:p-0">
        <DialogPrimitive.Content
          ref={ref}
          onInteractOutside={dismissOnOutside ? undefined : (event) => event.preventDefault()}
          style={{
            transform: dragOffset > 0 ? `translateY(${dragOffset}px)` : undefined,
            transition: isSnapping ? "transform 200ms var(--ease-out-quart)" : undefined,
          }}
          className={cn(
            ...
            // Шит: геометрия и своя пара анимаций (перебивает zoom/fade подменой animation-name)
            "max-md:max-w-none max-md:rounded-b-none max-md:max-h-[calc(100dvh-2.5rem)] max-md:pb-safe",
            "max-md:data-[state=open]:[animation-name:sheet-in] max-md:data-[state=open]:duration-300 max-md:data-[state=open]:ease-drawer",
            "max-md:data-[state=closed]:[animation-name:sheet-out] max-md:data-[state=closed]:duration-200",
            className
          )}
          {...props}
        >
          {/* Ручка шита: зона захвата 28px во всю ширину. Показывается только там,
              где свайп разрешён — ручка без жеста была бы ложным аффордансом. */}
          {dismissOnOutside && (
            <div
              className="md:hidden absolute inset-x-0 top-0 z-10 flex h-7 cursor-grab touch-none items-center justify-center"
          ...
          <DialogPrimitive.Close ref={closeProxyRef} tabIndex={-1} aria-hidden className="hidden" />
